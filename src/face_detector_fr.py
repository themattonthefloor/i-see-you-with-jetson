...
def furnish_image(frame, faces, names):
    """Draws bounding boxes & prints detection confidence.
...
def init_known_faces(dir="data", pkl_filename="faces.pkl"):
    ...
    Returns:
        Pandas DataFrame: Dataframe of names, encoding
    """
...
def save_known_faces(df, dir="data", filename="faces.pkl"):
    ...
    Args:
        df (Pandas DataFrame): Dataframe of names, encoding
...
def update_known_faces(df, name, enc, dir="data", filename="faces.pkl"):
    ...
    """
    new_row = {"name":name,"encoding":enc}
            ...
            if presence:
                ...
                for i, enc in enumerate(encs):
                    ...
                    if len(comparison)>0 and min(comparison)<args.tolerance:
                        name, _ = get_details(known_faces_df, np.argmin(comparison))
                        names.append(name)
                    else: # Update name, encoding, facial image file
                        name = "Unknown"
                        names.append(name)
                        known_faces_df = update_known_faces(known_faces_df, name, enc)
                ...
                # Furnish image
                frame = furnish_image(frame, faces, names)

            # Display output
            cv2.imshow(window_title, frame)

            if args.verbose:
                print(f"Frame time: {time()-frame_start_time:.3f}s | Face: {faces}")
